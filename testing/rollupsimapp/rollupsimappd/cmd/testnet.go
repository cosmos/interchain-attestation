...
Example:
	rollupsimappd testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "rollupsimappd", "Home directory of the node's daemon configuration")
...
Example:
	rollupsimappd testnet --v 4 --output-dir ./.testnets
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func initTestnetFiles(
	...
	rollupsimappConfig := srvconfig.DefaultConfig()
	rollupsimappConfig.MinGasPrices = args.minGasPrices
	rollupsimappConfig.API.Enable = true
	rollupsimappConfig.Telemetry.Enabled = true
	rollupsimappConfig.Telemetry.PrometheusRetentionTime = 60
	rollupsimappConfig.Telemetry.EnableHostnameLabel = false
	rollupsimappConfig.Telemetry.GlobalLabels = [][]string{{"chain_id", args.chainID}}
	...
	for i := 0; i < args.numValidators; i++ {
		...
		srvconfig.WriteConfigFile(filepath.Join(nodeDir, "config", "app.toml"), rollupsimappConfig)
	}
...
func startTestnet(cmd *cobra.Command, args startArgs) error {
	networkConfig := network.DefaultConfig(rollupsimapp.NewTestNetworkFixture)
