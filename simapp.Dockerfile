...
RUN --mount=type=cache,mode=0755,target=/go/pkg/mod cd simapp && go mod download

RUN --mount=type=cache,mode=0755,target=/go/pkg/mod cd simapp && make build

FROM alpine:3.18

RUN apk add --no-cache jq

COPY --from=builder /go/simapp/build/simappd /bin/simappd
